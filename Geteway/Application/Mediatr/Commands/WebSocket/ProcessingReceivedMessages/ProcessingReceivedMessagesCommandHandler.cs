    ...
    IContext context) : IRequestHandler<ProcessingReceivedMessagesCommand>
{
    static ProcessingReceivedMessagesCommandHandler()
    {
        ...
        OutputMappers = new()
        {
            ...
            { PurposeOfTheMessage.ConnectBattle, responseModel => (responseModel as ConnectCommandModel)?.MapToCurrentMapStatusDto() ?? throw BR },
            { PurposeOfTheMessage.DisconnectBattle, responseModel => responseModel is Unit ? new EmptyDto() : throw BR },
            { PurposeOfTheMessage.MakeStep, responseModel => responseModel is Unit ? new EmptyDto() : throw BR }
        };
    }
