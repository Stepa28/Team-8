...
public class MessageService(ILogger<MessageService> logger, ISender mediator) : IMessageService
{
    ...
    public async Task ProcessUserAddMessage(UserShortDto dto, CancellationToken cancellationToken = default) =>
        await mediator.Send(new AddUserCommand(dto), cancellationToken);

    public async Task ProcessUserDeleteMessage(UserShortDto dto, CancellationToken cancellationToken = default) =>
